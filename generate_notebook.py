...
cells.append(cell_code([
    ...
    "        colored_hm, superimposed = blend_gradcam(path, heatmap)\n",
    "        \n",
    "        cols_data = [cv2.resize(cv2.cvtColor(cv2.imread(path), cv2.COLOR_BGR2RGB), (224,224)), \n",
    "                     colored_hm, \n",
    "                     superimposed]\n",
    "        ttls = ['Image Originale', 'Heatmap (Saliency)', 'Superposition (Grad-CAM)']\n",
    "        for col,(d,t) in enumerate(zip(cols_data,ttls)):\n",
    "            ax_target = axes[row][col] if len(demo_paths) > 1 else axes[col]\n",
    "            ax_target.imshow(d)\n",
    "            ax_target.set_title(f'{t}\\n{cls.split(\"___\")[-1]}', fontsize=9)\n",
    "            ax_target.axis('off')\n",
